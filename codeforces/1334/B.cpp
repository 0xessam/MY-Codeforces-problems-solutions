...
void solllve()
{
    ...
    {
        if (i>=k) have+=i-k , now++ ;
        else need.push_back(k-i) ;
    }
    std::sort(need.begin(), need.end()) ;
    for (int i=1 ; i<need.size() ; i++) need[i]+=need[i-1] ;
    cout<< std::upper_bound(need.begin(), need.end(),have)-need.begin() + now<< el ;
