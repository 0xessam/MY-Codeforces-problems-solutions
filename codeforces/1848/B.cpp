...
{
    int n , k ; cin >> n >> k ;
    vector<vector<int>>v(k+5) ;
    vector<vector<int>>mx(k+3) ;
    for (int i = 1; i <= n ; ++i) {
        int x ; cin >> x;
        v[x].push_back(i) ;
    }
    for(int i=1 ; i<= k ; i++)
    {
        ...
        mx[i].push_back(v[i].front()-1) ;
        for (int j = 0; j < v[i].size()-1 ; ++j) {
            mx[i].push_back(v[i][j+1] - v[i][j]-1) ;
        }
        mx[i].push_back(n-v[i].back()) ;
    }
    int mn = 999999 ;
    for (int i = 1; i <= k  ; ++i) {
        vector<int>now = mx[i] ;
   ...
   //     cout << el ;
        ...
        if(now.size()>1)
        minz( mn , max(now.back()/2 , now[now.size()-2])) ;
        else
        minz( mn , now.back()/2) ;
    }
   // cout << el ;
