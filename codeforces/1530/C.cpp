...
bool val(int m , vector<int>a , vector<int> b)
{
    FOR(i,m) a.push_back(100) ;
    FOR(i,m) b.push_back(00) ;
    int k = a.size()-a.size()/4 ;
    rsrt(a) ;
    rsrt(b) ;
    return std::accumulate(a.begin(), a.begin()+k , 0ll)>= std::accumulate(b.begin(), b.begin()+k,0ll) ;
