...
{
    int dp,a,b,c,d ;
    cout << "? " << 1 << ' ' << 2 << el ; cout.flush() ; cin >> a ;
    cout << "? " << 1 << ' ' << 3 << el ; cout.flush() ; cin >> b ;
    cout << "? " << 1 << ' ' << 4 << el ; cout.flush() ; cin >> c ;
    cout << "? " << 1 << ' ' << 5 << el ; cout.flush() ; cin >> d ;
    int gc = __gcd(a,b) ;
    gc=__gcd(gc,c) ; gc=__gcd(gc,d) ;

    for (auto i : {4,8,15,16,23,42})
    {
        if (gc%i==0) {
            dp = i;
        }
    ...
    d/=gc ;
    cout << "! " ;
    for (auto i : {dp,a,b,c,d,(4+8+15+16+23+42-dp-a-b-c-d)}) cout << i << ' ' ;
