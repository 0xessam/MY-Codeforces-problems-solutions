...
{
    int c = 0 ;
    while (s.back()!='5' and s.empty()==0) s.pop_back(),c++ ;
    if (s.empty()) return io ;
    reverse(s.begin(), s.end()) ;
    for (int i=0 ; i<s.size() ; i++)
    {
        if (s[i]=='2' or s[i]=='7') return c+i-1 ;
    }
    return io ;
}
int dfs(string s)
{
    int c = 0 ;
    while (s.back()!='0' and s.empty()==0 ) s.pop_back(),c++ ;
    std::reverse(s.begin(), s.end()) ;
    for (int i=1 ; i<s.size() ; i++)
    {
        if (s[i]=='0' or s[i]=='5') return c+i-1 ;
    }
    return io ;
